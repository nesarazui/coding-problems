...
const leetTranslator = (string) => {
  ...
  for (let i = 0; i < str.length; i++) {
    let element = str[i] //loops through each letter in string
    console.log('letter is: ' + element)
    let index = letters.indexOf(element) //finds index number of that element in letters
    //console.log(index)
    translated += leetChars[index]
  }
  return translated;
}
